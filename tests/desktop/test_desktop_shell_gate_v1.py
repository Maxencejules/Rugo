"""M52 aggregate gate: desktop shell wiring and closure checks."""
...
def test_desktop_shell_gate_v1_wiring_and_artifacts():
    required = [
    ...
    for rel in required:
        assert (ROOT / rel).is_file(), f"missing M52 artifact: {rel}"
    ...
    assert "test-desktop-shell-v1" in makefile
    for entry in [
    ...
    assert "pytest-desktop-shell-v1.xml" in makefile
    assert "pytest-desktop-workflows-v1.xml" in makefile

    assert "Desktop shell v1 gate" in ci
    assert "make test-desktop-shell-v1" in ci
    assert "desktop-shell-v1-artifacts" in ci
    assert "out/pytest-desktop-shell-v1.xml" in ci
    ...
    assert shell_data["schema"] == "rugo.desktop_shell_workflow_report.v1"
    assert shell_data["contract_id"] == "rugo.desktop_shell_contract.v1"
    assert shell_data["gate_pass"] is True
    assert shell_data["workflow_totals"]["passed"] == 4

    assert installer_data["schema"] == "rugo.graphical_installer_smoke_report.v1"
    assert installer_data["workflow_id"] == "rugo.graphical_installer_flow.v1"
