...
def test_c_program_runs_against_rlibc(qemu_go_c4_runtime, find_in_order):
    ...
    find_in_order(out, [
        ...
        # rlibc v2 (Part V.11): errno now carries the kernel's DISTINCT code -- a
        # missing file is ENOENT=2 (not the old single EIO sentinel) -- and the
        # string helpers work (strcpy+strcat -> "rugo", strchr -> "go", atoi).
        "HELLOC: errno bad=-1 errno=2",
