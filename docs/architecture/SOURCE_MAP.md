...
| W^X on dynamic user memory (EFER.NXE + NX data pages) | `kernel_rs/src/mm.rs` (`enable_nx`, NX in the demand mapper), `apps/coreutils/nxprobe.asm` | `make test-wx-v1`, `tests/runtime/test_wx_runtime_v1.py` | live runtime | A stack-execution probe dies with the instruction-fetch fault code; the system shuts down cleanly. |
| Signals: handler delivery, sigreturn, default kill | `kernel_rs/src/lib.rs` (`sys_signal_ctl` id 48, `sig_deliver_if_pending`), `apps/coreutils/sigprobe.asm` | `make test-signals-v1`, `tests/runtime/test_signals_runtime_v1.py` | live runtime | Contract: `docs/runtime/signals_v1.md`. Frame-rewrite delivery; the probe handles signal 15 and resumes; unhandled signals kill. |
| Users/permissions: per-task uid, per-node owner+mode | uid in `kernel_rs/src/lib.rs`, owner/mode in `kernel_rs/src/vfs.rs`, `apps/coreutils/fsperm.asm`, `fschmod` in `services/go/fsh.go` | `make test-users-v1`, `tests/runtime/test_users_runtime_v1.py` | live runtime | Contract in `docs/runtime/vfs_v1.md`. A uid-100 probe is denied write/unlink on a root file until a root chmod. |
| SMP groundwork: AP bring-up, check-in, park | `kernel_rs/src/smp.rs` | `make test-smp-v1`, `tests/runtime/test_smp_runtime_v1.py` | live runtime | Contract: `docs/runtime/smp_v1.md`. All APs run kernel code on `-smp 4`; the default lane boots clean on multicore. Scheduling stays on the BSP. |
...
| Coreutils as external programs: ls/cat/echo/ps with spawn arguments | `apps/coreutils/*.asm`, args delivery in `sys_spawn_v1`, shell dispatch in `services/go/coreutils.go` | `make test-coreutils-v1`, `tests/runtime/test_coreutils_runtime_v1.py` | live runtime | Every output line comes from the spawned ELF itself; `ps` enumerates live tasks via `sys_proc_info`; `cat file | wc` joins two programs through a kernel pipe (`make test-pipes-v1`). |
| rlibc: POSIX-ish C layer + C programs from the package store | `libc/` (crt0, rlibc.c, headers), `tools/pe_to_elf_v1.py`, `apps/hello-c/` | `make test-libc-v1`, `tests/runtime/test_libc_runtime_v1.py` | live runtime | Contract: `docs/runtime/libc_v1.md`. A gcc-compiled C program runs from disk: printf, malloc, open/read on `/data`. |
| VFS with directories: writable `/data` tree over SimpleFS v2 | `kernel_rs/src/vfs.rs`, `/data` routing + `sys_fs_ctl` (id 47) in `kernel_rs/src/lib.rs`, `services/go/fsh.go` | `make test-vfs-v1`, `tests/runtime/test_vfs_runtime_v1.py` | live runtime | Contract: `docs/runtime/vfs_v1.md`. Create/write/list/stat/unlink with reboot persistence. |
| Wire TCP over the VirtIO NIC | `kernel_rs/src/tcp.rs`, RX pump + AF_INET socket diversion in `kernel_rs/src/net.rs`, `services/go/netcheck_tcp.go` | `make test-tcp-v1`, `tests/runtime/test_tcp_runtime_v1.py` | live runtime | Contract: `docs/runtime/tcp_v1.md`. Payload round-trips with a host-side listener through QEMU's user network. |
| DHCP + DNS clients over wire UDP | `kernel_rs/src/netcfg.rs` (`sys_net_query` id 49), `services/go/netcfgcheck.go` | `make test-netcfg-v1`, `tests/runtime/test_netcfg_runtime_v1.py` | live runtime | Contract: `docs/runtime/netcfg_v1.md`. Real DISCOVER/OFFER against QEMU's DHCP server; a real A query answered by the test's resolver. |
| Local console: framebuffer text output + PS/2 keyboard input | `kernel_rs/src/fb.rs`, `kernel_rs/src/kbd.rs`, console routing in `kernel_rs/src/lib.rs` | `make test-console-v1`, `tests/runtime/test_console_runtime_v1.py` | live runtime | Contract: `docs/runtime/console_v1.md`. A session typed via emulated PS/2; transcript verified as pixels by QMP screendump. |
