...
| Coreutils as external programs: ls/cat/echo/ps with spawn arguments | `apps/coreutils/*.asm`, args delivery in `sys_spawn_v1`, shell dispatch in `services/go/coreutils.go` | `make test-coreutils-v1`, `tests/runtime/test_coreutils_runtime_v1.py` | live runtime | Every output line comes from the spawned ELF itself; `ps` enumerates live tasks via `sys_proc_info`. Pipes pending. |
