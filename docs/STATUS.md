...
| **G1** Go services (TinyGo) | n/a | ✅ | Rugo: `tests/go/test_go_user_service.py` (`GOINIT: start`, `GOSVCM: start`, `GOSH: reply ok`, `TIMESVC: time ok`). TinyGo bare-metal x86_64. |
| **G2** Full Go port | n/a | ✅ | Rugo-only. Done: `tests/go/test_std_go_binary.py` and `make smoke-std` on the supported `go_std_test` stock-Go artifact path (`os-go-std.iso`, `GOSTD: ok`). |
...
| **M28** Security Hardening Program v3 | n/a | done | Rugo: hardening profile/threat model contracts + booted-runtime-bound attack evidence, deterministic fuzz and vulnerability-response artifacts, `make test-security-hardening-v3`, `make test-vuln-response-v1`, CI `Security hardening v3 gate` + `Vulnerability response v1 gate`, docs in `docs/security/hardening_profile_v3.md`, `docs/security/threat_model_v2.md`, and `docs/M28_EXECUTION_BACKLOG.md`. |
...
| **M32** Conformance + Profile Qualification v1 | n/a | done | Rugo: profile conformance contract + runtime-backed qualification against booted runtime, release attestation, and package rebuild evidence, `make test-conformance-v1`, CI `Conformance v1 gate`, docs in `docs/runtime/profile_conformance_v1.md`, and `docs/M32_EXECUTION_BACKLOG.md`. |
| **M33** Fleet-Scale Operations Baseline v1 | n/a | done | Rugo: fleet update/health and rollout-safety policy contracts + runtime-backed fleet/canary/abort control-plane reports, `make test-fleet-ops-v1`, `make test-fleet-rollout-safety-v1`, CI `Fleet ops v1 gate` + `Fleet rollout safety v1 gate`, docs in `docs/pkg/fleet_update_policy_v1.md`, `docs/runtime/fleet_health_policy_v1.md`, and `docs/M33_EXECUTION_BACKLOG.md`. |
| **M34** Maturity Qualification + LTS Declaration | n/a | done | Rugo: maturity qualification/LTS declaration contracts + runtime-backed cross-domain qualification bundle with bounded default-lane LTS scope, `make test-maturity-qual-v1`, CI `Maturity qualification v1 gate`, docs in `docs/build/maturity_qualification_v1.md`, `docs/build/lts_declaration_policy_v1.md`, and `docs/M34_EXECUTION_BACKLOG.md`. |
...
- `make image-go-std`, `make boot-std`, and `make smoke-std` form the
  supported stock-Go boot lane for `os-go-std.iso`.
...
M46 execution update (2026-03-10):
- PR-1 complete (bare-metal I/O contract freeze):
...
- PR-2 complete (deterministic wired-NIC, USB input, and removable-media tooling):
...
- PR-3 complete (bare-metal I/O gate + USB/removable sub-gate wiring):
  - `tests/hw/test_baremetal_io_gate_v1.py`
  - `tests/hw/test_usb_input_removable_gate_v1.py`
  - `Makefile` targets `test-baremetal-io-baseline-v1`,
    `test-usb-input-removable-v1`
  - `.github/workflows/ci.yml` steps `Bare-metal io baseline v1 gate`,
...
M49 execution update (2026-03-11):
- PR-1 complete (seat/input contract freeze):
...
- PR-2 complete (deterministic seat runtime + HID event-path tooling):
...
- PR-3 complete (input seat gate + HID event sub-gate wiring):
  - `tests/desktop/test_input_seat_gate_v1.py`
  - `tests/desktop/test_hid_event_path_gate_v1.py`
  - `Makefile` targets `test-input-seat-v1`,
    `test-hid-event-path-v1`
  - `.github/workflows/ci.yml` steps `Input seat v1 gate`,
    `HID event path v1 gate`
- M49 is done.

M50 execution update (2026-03-11):
- PR-1 complete (window-system contract freeze):
...
- PR-2 complete (deterministic window runtime + compositor damage tooling):
...
- PR-3 complete (window-system gate + compositor-damage sub-gate wiring):
  - `tests/desktop/test_window_system_gate_v1.py`
  - `tests/desktop/test_compositor_damage_gate_v1.py`
  - `Makefile` targets `test-window-system-v1`,
    `test-compositor-damage-v1`
  - `.github/workflows/ci.yml` steps `Window system v1 gate`,
